...
<head>
    ...
    <title>TOTTUS Sección de Alimentos Mejorada</title>
    <link rel="stylesheet" href="{{ asset('css/style.css') }}">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600&display=swap" rel="stylesheet">
</head>
    {{-- Botón Flotante de WhatsApp --}}
    <a href="https://wa.me/51939030861?text=Hola,%20me%20interesa%20saber%20más%20sobre%20sus%20productos." class="whatsapp-float" target="_blank" rel="noopener noreferrer">
        <img src="{{ asset('images/icons/whatsapp1.png') }}" alt="WhatsApp" class="whatsapp-icon-float">
    </a>
...
<body>
    ...
    <nav class="categorias-horizontal">
        <ul class="categorias-lista-horizontal">
    ...
    <main class="main-content">
        <section class="contenedor-banner-principal">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Asegúrate de que los botones existan antes de añadir listeners
            if (prevBtn && nextBtn) {
            ...
            if (bannerSlider) { // Asegurarse de que el slider exista
            ...
            // Script para mostrar la alerta del carrito
            const addToCartButtons = document.querySelectorAll('.boton-comprar');
            const cartAlert = document.getElementById('alerta-carrito');

            if (cartAlert) { // Asegúrate de que el elemento exista
                addToCartButtons.forEach(button => {
                    button.addEventListener('click', () => {
                        cartAlert.style.display = 'block';
                        // Opcional: Reiniciar la animación si se hace clic varias veces
                        cartAlert.classList.remove('fadeInOut');
                        void cartAlert.offsetWidth; // Trigger reflow
                        cartAlert.classList.add('fadeInOut');

                        setTimeout(() => {
                            cartAlert.style.display = 'none';
                            cartAlert.classList.remove('fadeInOut'); // Limpiar la clase para futuras animaciones
                        }, 4000); // Coincide con la duración de la animación en CSS
                    });
                });
            }
        });
